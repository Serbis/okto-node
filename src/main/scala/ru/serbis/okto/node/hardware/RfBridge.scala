...
object RfBridge {
  ...
  /** @param socket path to the unix domain socket control file (created by WSD)
    ...
    */
  def props(socket: String, maxReq: Int, cleanerTime: FiniteDuration = 1 second, nap: NativeApiProxy, testMode: Boolean = false) =
    Props(new RfBridge(socket, maxReq, cleanerTime, nap, testMode))
...
class RfBridge(socket: String, maxReq: Int, cleanerTime: FiniteDuration, nap: NativeApiProxy, testMode: Boolean = false) extends Actor with StreamLogger with Timers {
