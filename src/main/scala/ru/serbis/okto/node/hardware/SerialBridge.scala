...
import ru.serbis.okto.node.hardware.packets.{EventConfirmator, ExbPacket}
...
object SerialBridge {
  ...
  /** @param device path to the serial I / O device. For example '/def/ttyUSB0'
    ...
    */
  def props(device: String, baud: Int,
            maxReq: Int,
...
class SerialBridge(device: String,
                   baud: Int,
                   maxReq: Int,
                   cleanerTime: FiniteDuration = 1 second,
                   nap: NativeApiProxy,
  ...
  override def receive = {
    ...
    case EventConfirmator.Responses.EventAck(tid, addr, result) =>
      nap.serialPuts(serfd, ExbEventAckPacket(tid, if (result) 0 else 1).toArray)
    ...
    case UartPayload(data) =>
      ...
      packet match {
        ...
        case ep: ExbEventPacket if ep.confirmed =>
          val event = HardwareEvent(ep.eid, ep.tid, 0, confirmed = true, ep.payload)
        ...
        case ep: ExbEventPacket if !ep.confirmed =>
          val event = HardwareEvent(ep.eid, ep.tid, 0, confirmed = false, ep.payload)
