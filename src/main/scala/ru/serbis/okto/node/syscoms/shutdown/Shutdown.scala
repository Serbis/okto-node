package ru.serbis.okto.node.syscoms.shutdown
...
class Shutdown(env: Env, args: Vector[String], systemEx: ActorSystemExpander, testMode: Boolean) extends CmdExecutor(systemEx, testMode) {
  import Shutdown.States._
  import Shutdown.StatesData._
  import Shutdown.Internals._

  setLogSourceName(s"Shutdown*${self.path.name}")
  setLogKeys(Seq("Shutdown"))
  ...
  when(Idle, 5 second) {
    ...
    /** Send shutdown command to the nsd bridge and go to waiting response from it */
    case Event(req: CommandsUnion.Commands.Run, _) =>
      implicit val logQualifier = LogEntryQualifier("Idle_Run")
      process = req.process
      streams = req.streams

      env.nsdBridge ! NsdBridge.Commands.SendCommand("shutdown")
  ...
  when(CompleteExecution, 60 second) {
    case Event(Complete(code, message), _) =>
      ...
      logger.info(s"Command 'shutdown ${args.slice(1, args.size).toSpacedString}' completed with code $code ${if (code != 0) s" / $message" else ""}")
