package ru.serbis.okto.node.unit.syscoms.reboot
...
class RebootSpec extends TestKit(ActorSystem("TestSystem")) with ImplicitSender
  ...
  "Reboot" must {
    "Process positive test" in {
      ...
      val target = system.actorOf(Reboot.props(Env(nsdBridge = nsdBridge.ref), Vector.empty, null))

      probe.send(target, CommandsUnion.Commands.Run(process.ref,  Map(0 -> stdOut.ref, 1 -> stdIn.ref)))
      nsdBridge.expectMsg(NsdBridge.Commands.SendCommand("reboot"))
    ...
    "Return error if boot repository does not respond with expected timeout" in {
      ...
      val nsdBridge = TestProbe()
      val process = TestProbe()
      val target = system.actorOf(Reboot.props(Env(nsdBridge = nsdBridge.ref), Vector.empty, null, testMode = true))
